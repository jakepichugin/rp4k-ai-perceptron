...
class Perceptron(nn.Module): # Perceptron is a child of nn.module
    ...
    def forward(self, x): # x is the input tensor, that takes both the x and y
        ...
        return torch.sigmoid(self.fcOut(x)) # sigmoid activation function for rapidly moves between 0 and 1
...
def train_model():
    ...
    optimizer = optim.SGD(model.parameters(), lr=0.1) # lr is learning rate
...
def plot_decision_boundary(m, position_min, position_max):
    ...
    #s = 100 sets the size
    plt.scatter(np.array(points)[:,0], np.array(points)[:,1], c=labels, cmap="RdBu", edgecolors="k", s=100)
